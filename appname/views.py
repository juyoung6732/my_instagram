...
from .models import Post, CustomUser, Hashtag, Profile
from .forms import PostForm, SigninForm, UserForm, CommentForm, HashtagForm, ProfileForm
...
def mypage(request,pk):
    ...
    if user.is_authenticated:
        ...
        avartar = Profile.objects.filter(itsme=pk).last()
        if request.method == 'POST':
            if profile.is_valid():
            ...
            return render(request, 'appname/mypage.html', {'profile': profile, 'posts': posts, 'comment_form': comment_form, 'avartar':avartar})
    else:
        return HttpResponse('First SignIn please')
